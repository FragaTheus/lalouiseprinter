...
class StaffServiceTest {
    ...
    void createStaffShouldSaveAndReturnStaffWhenInputIsValid() {
        ...
        when(restaurantService.getRestaurant(restaurantId)).thenReturn(activeRestaurant());
        when(sectorService.getSector(sectorId)).thenReturn(activeSector());
    ...
    void createStaffShouldThrowWhenEmailAlreadyExists() {
        ...
        ManagerAlreadyExists ex = assertThrows(
                ManagerAlreadyExists.class,
                () -> service.createStaff(command)
        );
    ...
    @Test
    void getUserShouldThrowWhenContextIsNull() {
        RestaurantContext.clear();
        UUID id = UUID.randomUUID();
        when(staffRepository.findByIdAndRestaurantId(id, null)).thenReturn(Optional.empty());
    ...
    @Test
    void getAllShouldReturnEmptyPageWhenNoStaffsExist() {
        Pageable pageable = PageRequest.of(0, 10);
        Page<Staff> emptyPage = new PageImpl<>(List.of(), pageable, 0);

        when(staffRepository.findAllStaffs(null, null, restaurantId, pageable)).thenReturn(emptyPage);
    ...
    void getAllShouldFilterByTermAndActive() {
        ...
        when(staffRepository.findAllStaffs("john", true, restaurantId, pageable)).thenReturn(page);

        Page<Staff> result = service.getAll("john", true, pageable);

        assertEquals(1, result.getContent().size());
        verify(staffRepository).findAllStaffs("john", true, restaurantId, pageable);
    }
}
